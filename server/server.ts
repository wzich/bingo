...
db.exec(
  "CREATE TABLE IF NOT EXISTS board (board_tile_id INTEGER PRIMARY KEY AUTOINCREMENT, game_id STRING not null, board_id STRING not null, player_id INTEGER not null, tile_id INTEGER not null)",
);
...
app.get("/board/:game_name/:player_id", async (c) => {
  const name = await c.req.param("game_name");
  const player_id = await c.req.param("player_id");

  function getBoard({ name, player_id }: { name: string; player_id: string }) {
    return db.prepare(
      "SELECT * FROM board INNER JOIN tile WHERE board.game_id = :name AND board.player_id = :player_id",
    ).all({ name, player_id });
  }

  const existing_board = getBoard({ name, player_id });
  ...
  db.prepare(
    "INSERT INTO board (board_id, game_id, player_id, tile_id) SELECT :board_id, :name, :player_id, tile_id FROM tile INNER JOIN game ON game.game_id = tile.game_id WHERE game.name = :name ORDER BY RANDOM() LIMIT 16;",
  ).all({ board_id, player_id, name });
  const new_board = getBoard({ name, player_id });
  return c.json(new_board);
});
